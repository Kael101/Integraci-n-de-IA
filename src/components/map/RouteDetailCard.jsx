...
import { X, Clock, Mountain, Footprints, Play, Layers, Sparkles } from 'lucide-react'; // Added Layers, Sparkles
import JIcon from '../ui/JIcon';
import ArchaeologyViewer from '../ar/ArchaeologyViewer';

const RouteDetailCard = ({ route, onClose }) => {
    const [showArqueologia, setShowArqueologia] = useState(false);

    if (!route) return null;

    return (
        <>
            {/* VISOR ARQUEOLOGÍA (Overlay Global) */}
            {showArqueologia && (
                <ArchaeologyViewer onClose={() => setShowArqueologia(false)} />
            )}
            ...
            <div className="absolute bottom-[110px] left-4 right-4 z-30 animate-slide-up">
                ...
                <div className="bg-jaguar-950/90 backdrop-blur-xl border border-white/10 rounded-2xl p-5 shadow-2xl shadow-black/60 relative overflow-hidden">
                    ...
                    <div className="flex flex-col gap-3">
                        {/* Botón Arqueología Digital (Nuevo) */}
                        <button
                            onClick={() => setShowArqueologia(true)}
                            className="w-full flex items-center justify-center gap-2 bg-orange-900/40 hover:bg-orange-900/60 border border-orange-500/30 p-3 rounded-xl transition-all group"
                        >
                            <Layers size={16} className="text-orange-400 group-hover:animate-pulse" />
                            <div className="flex flex-col items-start leading-none">
                                <span className="text-[10px] text-orange-300 font-bold uppercase tracking-wider">Modo Exploración</span>
                                <span className="text-sm font-bold text-white">Ver Ciudad Perdida (LiDAR)</span>
                            </div>
                        </button>
